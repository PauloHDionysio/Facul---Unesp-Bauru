...
function salvaValor(){
    ...
    while((resultadoSimplex===0  || flagInteiro===0) && count<8){
        count++;
        ...
        if(count===8){
            ...
            if(flagInteiro===0){
                ...
                if(linhaNaoInteira !== 0){
                    criaNovaRestricao(linhaNaoInteira);

                    
                    flagGomory=1;
                    console.log("Matriz antes do simplex gomory:")
                    for(let linha=0; linha<numRestr+2; linha++){
                        var array=[];
                        for(let coluna=0; coluna<numVariaveis+(numRestr*2)+2; coluna++){
                    ...
                    for(i=1; i<=numVariaveis; i++){
                        for(let j=0; j<=numRestr; j++){
                            if(vetorBase[j]===i){
                                if(Math.round(matrizPadrao[j][numVariaveis+(2*numRestr)+1]) - matrizPadrao[j][numVariaveis+(2*numRestr)+1].toFixed(6) !== 0) flagInteiro=0;
                            }
                        }
                    }

                    console.log("\n" + "Simplex Gomory - Iteracao " + count);
                    for(let linha=0; linha<numRestr+2; linha++){
                        var array=[];
                        for(let coluna=0; coluna<numVariaveis+(numRestr*2)+2; coluna++){
                            array[coluna]=matrizPadrao[linha][coluna];
...
function verificaInteiro(){
    var linhaRacional=0;
    for(linha=1; linha<numRestr+1; linha++){
        for(coluna=1; coluna<numVariaveis+(numRestr+2); coluna++){
            linhaRacional=linha;
            if(matrizPadrao[linha][coluna]%1 !== 0) return linhaRacional;
        }
    }
...
function criaNovaRestricao(linhaNaoInt){
    ...
    numRestr++;
    console.log("Linha que sera truncada: " + linhaNaoInt);
