...
void Newton_Dividido(int n, double tabela[][MAX], double x){
	...
	for(i=0 ; i<n-1 ; i++){
        ...
        SetColor(CINZA); cout << endl << "Polinomio " << i+1 << ": "; SetColor(BRANCO); cout << polinomio << endl;
	}
	cout << endl << endl;
...
void coefDeterminacao(double Y[], double Y_ajustado[], int n){
	...
	SetColor(VERDE); cout << endl << "Coeficiente de determinacao: " << R2 << endl;
}
...
void mostra(double a0, double a1, double Y_ajustado[], int n){
	...
	SetColor(CINZA); cout << endl << endl << endl << "Vetor y ajustado a reta (Y = " << a0 << " + " << a1 << "*x):" << endl;
	SetColor(BRANCO);
...
void mostraExpo(double a0, double a1, double Y_ajustado[], int n){
	...
	SetColor(CINZA); cout << endl << endl << "Vetor Y ajusatado exponencial:" << endl; SetColor(BRANCO);
	for(i=0 ; i<n ; i++) cout << Y_ajustado[i] << "  ";
	cout << endl;
}
...
int main(){
	...
	while(ON==true){
		...
		while(pos==2){ //2.2 - Polinomio interpolado por Newton-Gregory
            ...
            if(h!=0) Newton_Gregory(n, tabela, h, x);
			else{
				SetColor(VERMELHO);
				cout << "O h nao esta no intervalo limitado. Tente novamente.";
			}
